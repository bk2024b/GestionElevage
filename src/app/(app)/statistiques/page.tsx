...
export default async function StatistiquesPage() {
  ...
  return (
    <div className="max-w-md md:max-w-4xl mx-auto px-5 py-6">
      <h1 className="text-xl md:text-2xl font-bold mb-5">Statistiques</h1>

      <div className="grid grid-cols-3 gap-3 mb-4">
        <Card className="!p-4">
          <Heart size={20} strokeWidth={2} className="text-ink mb-2" />
          <p className="text-xs text-ink-soft">Reproduction</p>
          <p className="text-xl font-bold">{stats.tauxReproduction}%</p>
          <p className="text-xs text-ink-soft/70 hidden md:block">{stats.totalAccouplements} accouplements</p>
        </Card>
        <Card className="!p-4">
          <Skull size={20} strokeWidth={2} className="text-ink mb-2" />
          <p className="text-xs text-ink-soft">Mort. naissance</p>
          <p className="text-xl font-bold">{stats.tauxMortalite}%</p>
          <p className="text-xs text-ink-soft/70 hidden md:block">{stats.totalMorts} / {stats.totalNes} nés</p>
        </Card>
        <Card className="!p-4">
          <Baby size={20} strokeWidth={2} className="text-ink mb-2" />
          <p className="text-xs text-ink-soft">Mort. sevrage</p>
          <p className="text-xl font-bold">{stats.tauxMortaliteSevrage}%</p>
      ...
      <div className="md:grid md:grid-cols-2 md:gap-4">
        <Card className="mb-4 md:mb-0">
          ...
          <div className="flex justify-between text-sm mb-2">
            ...
            <span className="text-accent-green font-medium">{formatFCFA(stats.revenus)}</span>
          </div>
          <div className="flex justify-between text-sm mb-2">
          ...
          <div className="flex justify-between text-sm border-t border-line pt-2 mt-1">
            ...
            <span className={`font-medium ${stats.benefice >= 0 ? 'text-accent-green' : 'text-danger'}`}>
        ...
        <Card className="mb-6 md:mb-0 flex flex-col justify-center items-center text-center">
          <Rabbit size={32} strokeWidth={1.6} className="text-ink mb-2" />
          <p className="text-xs text-ink-soft">Cheptel total</p>
          <p className="text-3xl font-bold">{stats.totalLapins}</p>
        </Card>
      </div>
      ...
      <div className="mt-8">
        <p className="text-sm font-semibold text-ink-soft mb-2">Performance par reproducteur</p>
